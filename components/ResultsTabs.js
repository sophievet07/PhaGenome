...
export default function ResultsTabs({ results }) {
  ...
  const { blast, phaster, annotation, trna, safety, validation } = results
    ...
    <>
      ...
      <div className="results-card">
        ...
        <div className="tabs-row">
          {['overview','annotation','tRNA','safety','genome map'].map(t => (
            <button
              key={t}
...
function downloadReport(r) {
  ...
  txt += `Platform: PhaGenome — ICAR-NMRI, Hyderabad\n\n`
  txt += `GENOME\n${'-'.repeat(40)}\n`
  txt += `Phage: ${r.blast?.topHit || 'Unknown'}\n`
  txt += `Taxonomy: ${r.blast?.taxonomy || '—'}\n`
  txt += `Size: ${(r.validation?.totalLength || 0).toLocaleString()} bp\n`
  txt += `GC%: ${r.validation?.gc || 0}%\n\n`
  txt += `LIFESTYLE: ${r.phaster?.lifestyle || '—'} (${r.phaster?.confidence || 0}% confidence)\n\n`
  txt += `ANNOTATION\nTotal ORFs: ${r.annotation?.total}\nFunctional: ${r.annotation?.functional}\n\n`
  txt += `tRNA GENES: ${r.trna?.count || 0}\n\n`
  txt += `SAFETY: ${r.safety?.overall}\n`
  txt += `AMR hits: ${r.safety?.amr?.hitsAboveThreshold || 0}\n`
  txt += `Toxin hits: ${r.safety?.toxin?.hitsAboveThreshold || 0}\n\n`
  txt += `${'='.repeat(60)}\nPhaGenome — ICAR-NMRI, Hyderabad, India\n`
  downloadText(txt, 'phagenome_report.txt')
}
